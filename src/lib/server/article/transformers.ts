...
const surroundCodeBlocks: HTMLTransformer = (html) => {
    ...
    return html.replace(/<figure([^>]*?)>(.*?)<\/figure>/gis, (_match: string, attributes: string, content: string) => {
        const figCaption = content.match(/<figcaption([^>]*?)>(.*?)<\/figcaption>/is)!;

        const language = figCaption
            .at(1)!
            .match(/data-language="(.*?)"/is)!
            .at(0)!
            .split('=')
            .at(1)!
            .replaceAll(/"+/g, '')!;
        ...
        const contentToCopy = apply(content, removeHtmlTags, replaceEntityWithSymbol);
