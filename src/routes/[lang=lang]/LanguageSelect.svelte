...
<div class="relative">
    <button bind:this={currentLanguageNode} on:click={() => (showList = !showList)} class="bg-gray-700 px-3 py-2 rounded {showList ? "rounded-b-none" : ""}">
    ...
    {#if showList}
        <div on:emit={() => (showList = false)} use:clickOutside={{ avoid: [currentLanguageNode] }} class="absolute w-full top-full left-0 flex flex-col z-20">
			{#if $locale === "en"}
				<button on:click={() => changeLang("fr")} class="relative w-full bg-gray-700 px-3 py-2 rounded {showList ? "rounded-t-none" : ""}">
					{$LL.languages.french()}
				</button>
			{:else}
				<button on:click={() => changeLang("en")} class="relative w-full bg-gray-700 px-3 py-2 rounded {showList ? "rounded-t-none" : ""}">
